...
export class PasajerosController {
  ...
  @ApiBody({
    ...
    description:
      'Datos del pasajero a crear (FormData). El campo documentacion debe ser un archivo.',
  })
  createPasajero(
    ...
    return this.pasajerosService.createPasajeros(
      createPasajeroDto,
      idUser,
      +cliente,
      documentacionFile,
  ...
  @ApiOperation({
    ...
    description:
      'Retorna la información principal del wallet incluyendo saldos, transacciones y estadísticas mensuales de gastos y recargas. El parámetro anio es opcional, si no se proporciona se usa el año actual.',
  })
  @ApiQuery({
    name: 'anio',
    required: false,
    type: Number,
    description:
      'Año para filtrar los arrays de gastosYRecargasPorMes y gastosPorMes. Si no se proporciona, se usa el año actual.',
    example: 2025,
  })
  findMainPasajero(@Request() req, @Query('anio') anio?: string) {
    const idUsuario = req.user.userId;
    const idUser = req.user.userId;
    const cliente = req.user.cliente;
    const rol = req.user.rol;
    const anioFiltro = anio ? parseInt(anio, 10) : undefined;
    return this.pasajerosService.obtenerMainPasajero(
      idUsuario,
      idUser,
      cliente,
      rol,
  ...
  @ApiOperation({
    ...
    description:
      'Actualiza el CustomerIdNetPay de Netpay asociado a un pasajero específico.',
  })
  @ApiResponse({
    status: 200,
    description: 'CustomerIdNetPay actualizado exitosamente',
  })
