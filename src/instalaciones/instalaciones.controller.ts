...
export class InstalacionesController {
  ...
  @ApiOperation({
    ...
    description:
      'Obtiene todas las instalaciones activas asociadas al validador especificado.',
  })
  @ApiParam({
    name: 'idValidador',
    type: Number,
    description:
      'ID del validador del cual se desean obtener las instalaciones',
    example: 1,
  })
  @ApiResponse({
    status: 200,
  ...
  async findByValidador(
    ...
    const idUser = req.user.userId;
    const rol = req.user.rol;
    return await this.instalacionesService.findByValidador(
      +idValidador,
      +idUser,
      +cliente,
      +rol,
